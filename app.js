...
async function loadHeroSlider() {
  ...
  const enriched = await Promise.all(
    movies.map(async m => ({ ...m, hero_backdrop: resolveHeroBackdrop(m, await getFanartBackdrop(m.id)) }))
  );
...
async function runSearch(q) {
  ...
  const url = buildTMDBUrl('/search/multi', { query: q, page: 1 });
  try {
